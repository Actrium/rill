...
TenantManager 是一个 C++ 原生模块,用于在单个应用程序内管理多个隔离的沙箱。每个沙箱称为一个 **tenant**,在专用线程上运行自己的 guest bundle,具有独立的资源配额、定时器和生命周期管理。
...
通过向 TenantManager 传递配置对象来创建 tenant:
...
tenant manager.isOverQuota(tenantId);   // 如果任何限制超出则返回 true
tenant manager.isNearQuota(tenantId);   // 如果任何限制 > 80% 则返回 true
```

### 违规跟踪

TenantManager 记录每个 tenant 的配额和策略违规:
