...
| **硬超时** | No | No | No (硬中断支持推进中) | No | Yes (`vm.Script` timeout) |
...
> **强制执行现状:** `timeout` 与 `memoryLimit` 虽在选项面上,但**目前均未被原生 JSI 引擎强制执行**。QuickJS、Hermes、JSC 都接收 `timeout` 值后直接丢弃(QuickJS 的 context 构造函数丢弃该参数;Hermes 与 JSC 标注为 "reserved for future use")。QuickJS 的硬中断超时支持正在推进中;Hermes 与 JSC 完全不强制超时。`memoryLimit` 同样未接线:QuickJS 原生引擎内部使用固定的 256 MB 堆上限,租户级 `maxHeapBytes` 配额也从未下发到任何引擎。目前只有 Node VM 真正强制 `timeout`(通过 `vm.Script`)。在此改变之前,原生引擎的 CPU 与堆限制应视为宿主侧的责任。
