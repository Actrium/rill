...
| **Hard timeout** | No (no public interrupt API) | Yes (`watchTimeLimit`) | Yes (interrupt handler) | No | Yes (`vm.Script` timeout) |
| **Heap quota (`maxHeapBytes`)** | No | No | Yes (`JS_SetMemoryLimit`) | No | No |
...
- Hard timeout enforced via `HermesRuntime::watchTimeLimit`: an expired
  budget makes the running eval throw instead of hanging the host thread.
- No heap quota API; `maxHeapBytes` is ignored.
...
- Hard timeout enforced via a QuickJS interrupt handler (wall-clock
  deadline); a tenant infinite loop is aborted with a timeout error.
- Heap quota enforced via `JS_SetMemoryLimit` (`maxHeapBytes`; 256MB default).
