...
| **Import** | *(internal)* `src/host/sandbox/providers/JSCProvider*` | *(internal)* `src/host/sandbox/providers/HermesProvider*` | *(internal)* `src/host/sandbox/providers/QuickJSProvider*` | *(internal)* `src/host/sandbox/providers/QuickJSNativeWASMProvider` | *(internal)* `src/host/sandbox/providers/NodeVMProvider` |
...
| **Hard timeout** | No | No | No (hard-interrupt support in progress) | No | Yes (`vm.Script` timeout) |
...
**Source:** `src/host/sandbox/providers/NodeVMProvider.ts`
...
| Node.js / Bun (dev, test, CI) | `NodeVMProvider` | Native speed, hard timeout, zero setup |
...
1. **NodeVMProvider** -- selected when `process.versions.node` is defined and `require('node:vm')` resolves.
...
> **Enforcement status:** `timeout` and `memoryLimit` are part of the option surface but are **not currently enforced by the native JSI engines**. QuickJS, Hermes, and JSC all accept the `timeout` value and then discard it (the QuickJS context constructor drops the parameter; Hermes and JSC mark it "reserved for future use"). Hard-interrupt timeout support for QuickJS is in progress; Hermes and JSC do not enforce timeouts at all. `memoryLimit` is likewise not wired through: the QuickJS native engine uses a fixed internal 256 MB heap limit, and the tenant-level `maxHeapBytes` quota is never delivered to any engine. Today only Node VM enforces `timeout` (via `vm.Script`). Until this changes, treat CPU and heap limits on the native engines as a host-side responsibility.
