...
>
> 本文既是设计，也记录**已落地的 Phase A + B**：宿主侧 ABI（`WasmGuestHost`）+ 人体工学 Rust guest SDK（`store::put(k,v).await`）+ 一个真 Rust `.wasm` guest 端到端往返 + 封口负向，全部有 `bun test` 跑绿。
...
| Rust guest SDK（future/executor/宏/typed 包装） | `crates/rill-guest/src/lib.rs` |
| 真 Rust guest（用 SDK） + 构建 | `crates/kv-guest/src/lib.rs`、`crates/build.sh` |
| 最小原生 guest（wire 级 ABI，手写） | `src/host/wasm-guest/__tests__/fixtures/roundtrip.wat` |
| 端到端 + 封口负向 + Rust guest 测试 | `src/host/wasm-guest/__tests__/wasm-guest-host.test.ts` |
