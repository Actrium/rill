...
>
> 本文既是设计，也记录**已落地的 Phase A**：宿主侧 ABI（`WasmGuestHost`）+ 一个真 `.wasm` guest 端到端往返 + 封口负向，全部有 `bun test` 跑绿。
...
**Phase B · 工具链**：Rust `rill-guest` crate 包住 ABI（开发者写 `store::get(key).await`，crate 处理 alloc/host_call/resolve/future）+ CLI 增加 `.wasm` guest 构建目标（现只出 JS bundle）；跑通 store/net。

**Phase C · 渲染 + 多语言**：原生 guest 发**渲染批次**（复用现有二进制 op 协议）→ receiver 物化；补 host:* 全集；扩 C/Zig SDK。

**消费者（之后）**：canvas 帧缓冲 / `host:gpu`、重计算 app……见 [rill-canvas.zh.md]（application.ist 侧）。

## 6. 渲染 / 事件（地基阶段只留接口）

原生 guest 出 UI 就**发渲染批次**（现有二进制 op 协议，Phase B 的 SDK 帮建）→ host receiver 照旧物化；输入 / 生命周期走 `rill_on_event(ptr,len)`（复用现有事件通道语义，Phase C 接入）。**canvas 的帧缓冲 present 是消费者层，地基不碰。**

## 7. 测试

Phase A 已带：ABI 端到端往返 + 封口负向（见 §5）。地基推进时按此纪律扩：
...
| 最小原生 guest（wire 级 ABI） | `src/host/wasm-guest/__tests__/fixtures/roundtrip.wat` |
| 端到端 + 封口负向测试 | `src/host/wasm-guest/__tests__/wasm-guest-host.test.ts` |
...
**一句话**：② = 把 QuickJS 早就在用的那条底层 host↔WASM 桥，**抽成语言中立的线性内存 ABI**，供原生 `.wasm` 直接绑；因**派发/broker/渲染/信任全共享、只多一个 guest 侧适配器**，所以**加法式兼容、不动现有 QuickJS**。Phase A 已用一个真 `.wasm` guest 把「装载 + host:* 往返 + 封口」端到端跑通。
