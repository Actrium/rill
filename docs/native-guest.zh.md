...
`wasm-guest-host.test.ts`（15 绿）。因这是**信任边界**（host 解引用 guest 控制的指针），覆盖不止 happy-path，还含对抗与不变量：

- **端到端**：手写最小 guest host:kv.put 往返；真 Rust guest `store::put().await`；渲染批次经真 receiver 物化成节点树。
