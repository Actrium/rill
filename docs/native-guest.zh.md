...
- **端到端**：手写最小 guest host:kv.put 往返；真 Rust guest `store::put().await`；渲染批次经真 receiver 物化成节点树；**host→guest 事件**（`emitEvent` → guest handler 收到 payload、名字过滤、重复投递、无导出则 no-op）；**C guest**（C SDK 写的 guest 经同一 host **渲染**成功 → ABI 语言中立在 C 上得证。**诚实注**：C SDK 首版只做单向 render 路径，`host:*` 异步往返/事件的语言中立目前**只在 Rust 上验证**，C 侧 executor 是 follow-up）。
