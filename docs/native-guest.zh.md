...
`wasm-guest-host.test.ts`（18 绿）。因这是**信任边界**（host 解引用 guest 控制的指针），覆盖不止 happy-path，还含对抗与不变量：

- **端到端**：手写最小 guest host:kv.put 往返；真 Rust guest `store::put().await`；渲染批次经真 receiver 物化成节点树；**host→guest 事件**（`emitEvent` → guest handler 收到 payload、名字过滤、重复投递、无导出则 no-op）。
...
- **封口不变量（自动化）**：对每个 guest wasm 用 `WebAssembly.Module.imports` 断言 import ⊆ host 提供集（`rill_host_call`/`rill_send_batch`/`rill_log`）——「无 fetch/socket」从肉眼 claim 变成回归守卫。

> 补对抗测试时**逼出并修掉一个真实重入 bug**：错误路径（缺 handler/坏 JSON/越界）曾同步 resolve，而 `rill_resolve` 会重入 guest executor → 重 poll 正在 poll 的 future → 栈溢出。修法：`onHostCall` 先让出一个 microtask，resolve 永不同步重入。

**仍缺（follow-up）**：fuzz（随机 op 流/畸形 props 的 property-based）、JS+原生 guest 同 host 共存、bump 分配器溢出、接平台后 `host:store` 活桥 e2e。
...
| 真 Rust guest：host:* 调用 / 连续 await / 渲染 / 收事件 | `crates/{kv,seq,ui,event}-guest/src/lib.rs`、`crates/build.sh` |
