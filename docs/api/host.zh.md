...
| 属性 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `sandbox` | `'node-vm' \| 'jsc' \| 'quickjs' \| 'hermes' \| 'wasm-quickjs' \| 'tenant-manager'` | 自动检测 | 明确选择沙箱后端。 |
| `tenant` | `TenantConfig` | `undefined` | 使用 TenantManager 沙箱时的租户配置。 |
...
用于监控与调试，`getDiagnostics()` 是唯一的公共入口：
