...
// static
jsi::Value JSIValueConverter::ToJSIValue(const QuickJSRuntime &runtime,
  ...
  if (JS_IsString(value)) {
    return QuickJSRuntime::make<jsi::String>(
        new QuickJSPointerValue(jsRuntime, jsContext, value));
  }
  if (JS_IsSymbol(value)) {
    return QuickJSRuntime::make<jsi::Symbol>(
        new QuickJSPointerValue(jsRuntime, jsContext, value));
  }
  if (JS_IsObject(value)) {
    return QuickJSRuntime::make<jsi::Object>(
        new QuickJSPointerValue(jsRuntime, jsContext, value));
  }
  if (JS_IsBigInt(jsContext, value) || JS_IsBigDecimal(value) ||
      JS_IsBigFloat(value)) {
    return QuickJSRuntime::make<jsi::BigInt>(
        new QuickJSPointerValue(jsRuntime, jsContext, value));
  }
...
JSValue JSIValueConverter::ToJSValue(const QuickJSRuntime &constRuntime,
...
JSValue JSIValueConverter::ToJSString(const QuickJSRuntime &runtime,
...
JSValue JSIValueConverter::ToJSString(const QuickJSRuntime &runtime,
  ...
  const QuickJSPointerValue *quickJSPointerValue =
      static_cast<const QuickJSPointerValue *>(
          runtime.getPointerValue(propName));
  auto jsObject = quickJSPointerValue->Get(runtime.getJSContext());
  assert(JS_IsString(jsObject));
  return jsObject;
}

JSValue JSIValueConverter::ToJSSymbol(const QuickJSRuntime &runtime,
...
JSValue JSIValueConverter::ToJSObject(const QuickJSRuntime &runtime,
...
JSValue JSIValueConverter::ToJSBigInt(const QuickJSRuntime &runtime,
...
JSValue JSIValueConverter::ToJSArray(const QuickJSRuntime &runtime,
...
JSValue JSIValueConverter::ToJSFunction(const QuickJSRuntime &runtime,
                                        const jsi::Function &function) {
  const QuickJSPointerValue *quickJSPointerValue =
      static_cast<const QuickJSPointerValue *>(
          runtime.getPointerValue(function));
  auto jsObject = quickJSPointerValue->Get(runtime.getJSContext());
...
jsi::PropNameID
JSIValueConverter::ToJSIPropNameID(const QuickJSRuntime &runtime,
  ...
  return runtime.make<jsi::PropNameID>(new QuickJSPointerValue(
      runtime.getJSRuntime(), runtime.getJSContext(), jsValue));
}

std::string JSIValueConverter::ToSTLString(JSContext *ctx, JSAtom atom) {
...
// static
std::string JSIValueConverter::ToSTLString(JSContext *context,
                                           JSValueConst &string) {
