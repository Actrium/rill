...
static void js_dump_obj(JSContext *ctx, JSValueConst val) {
  ...
  if (str) {
    std::cerr << "QuickJS exception " << str;
...
std::string urlToCacheKey(const std::string &uri) {
  ...
  if (uri.find("http") != std::string::npos &&
      std::regex_search(uri, path_match, dev_path_regex)) {
    return path_match[3];
  }
  ...
  if (std::regex_search(uri, path_match, iOS_path_regex) &&
      !path_match.empty()) {
    return path_match[0];
  }
  return "codecache";
...
void QuickJSRuntime::loadCodeCache(CodeCacheItem &codeCacheItem,
                                   const std::string &url, const char *source,
                                   size_t size) {
  ...
  // Code cache disabled - would require folly::readFile
  (void)codeCacheItem;
  (void)codeCachePath;
  (void)buffer;
  (void)url;
...
void QuickJSRuntime::updateCodeCache(CodeCacheItem &codeCacheItem,
                                     const std::string &url, const char *source,
                                     size_t size) {
  if (codeCacheDir_.empty()) {
    return;
  ...
  std::string codeCachePath = codeCacheDir_ + "/" + cacheKey;
  // Code cache disabled - would require folly::writeFile
  (void)codeCachePath;
  (void)codeCacheItem;
  (void)url;
  (void)source;
...
jsi::Value QuickJSRuntime::evaluateJavaScript(
  ...
  // Only enable code cache if we have a cache directory
  bool enableCodeCache = !codeCacheDir_.empty();
  JSValue retValue;
  ScopedJSValue scopedJsValue(context_, &retValue);

  if (enableCodeCache) {
    CodeCacheItem codeCacheItem;
    loadCodeCache(codeCacheItem, sourceURL, (const char *)buffer->data(),
                  buffer->size());
    bool hasCodeCache = (codeCacheItem.result == CodeCacheItem::INITIALIZED);
    JSValue func, cachedFunc;
    if (hasCodeCache) {
      func = JS_ReadObject(context_, codeCacheItem.data.get(),
                           codeCacheItem.size, JS_READ_OBJ_BYTECODE);
    } else {
      func = JS_Eval(context_, (const char *)buffer->data(), buffer->size(),
                     sourceURL.c_str(),
    ...
    if (!hasCodeCache) {
      ...
      if (buf && size != 0) {
        // Use js_free to properly free the buffer allocated by JS_WriteObject
        // We need to copy it to a buffer that unique_ptr can manage, or use
        // custom deleter For simplicity, we'll copy and free the original
        uint8_t *bufCopy = new uint8_t[size];
        memcpy(bufCopy, buf, size);
        js_free(context_, buf);
        codeCacheItem.data = std::unique_ptr<uint8_t>(bufCopy);
        codeCacheItem.size = size;
        codeCacheItem.result = CodeCacheItem::REQUEST_UPDATE;
        updateCodeCache(codeCacheItem, sourceURL, (const char *)buffer->data(),
                        buffer->size());
      } else {
    ...
    retValue = JS_Eval(context_, (const char *)buffer->data(), buffer->size(),
...
QuickJSRuntime::cloneSymbol(const Runtime::PointerValue *pv) {
...
jsi::Runtime::PointerValue *
QuickJSRuntime::cloneBigInt(const Runtime::PointerValue *pv) {
  // TRACE_SCOPE("QuickJSRuntime", "string");
  if (!pv) {
    return nullptr;
  }
...
jsi::Runtime::PointerValue *
QuickJSRuntime::cloneString(const Runtime::PointerValue *pv) {
  // TRACE_SCOPE("QuickJSRuntime", "string");
  if (!pv) {
    return nullptr;
  }
...
jsi::Runtime::PointerValue *
QuickJSRuntime::cloneObject(const Runtime::PointerValue *pv) {
...
jsi::Runtime::PointerValue *
QuickJSRuntime::clonePropNameID(const Runtime::PointerValue *pv) {
  // TRACE_SCOPE("QuickJSRuntime", "string");
...
QuickJSRuntime::getNativeState(const jsi::Object &object) {
  JSValue jsValue = JSIValueConverter::ToJSObject(*this, object);
  ScopedJSValue scopedJsValue(context_, &jsValue);
  auto opaqueData = reinterpret_cast<OpaqueData *>(JS_GetOpaqueUnsafe(jsValue));
  assert(opaqueData);
...
void QuickJSRuntime::setNativeState(const jsi::Object &object,
...
jsi::PropNameID QuickJSRuntime::createPropNameIDFromAscii(const char *str,
                                                          size_t length) {
  // TRACE_SCOPE("QuickJSRuntime", "string");
  JSValue jsValue = JS_NewStringLen(context_, str, length);
  ScopedJSValue scopedJsValue(context_, &jsValue);
  QuickJSPointerValue *value =
      new QuickJSPointerValue(runtime_, context_, jsValue);
  return make<jsi::PropNameID>(value);
}

jsi::PropNameID QuickJSRuntime::createPropNameIDFromUtf8(const uint8_t *utf8,
                                                         size_t length) {
  // TRACE_SCOPE("QuickJSRuntime", "string");
...
jsi::PropNameID
QuickJSRuntime::createPropNameIDFromString(const jsi::String &str) {
  // TRACE_SCOPE("QuickJSRuntime", "string");
  const QuickJSPointerValue *quickJSPointerValue =
...
bool QuickJSRuntime::compare(const jsi::PropNameID &a,
...
jsi::String QuickJSRuntime::createStringFromAscii(const char *str,
                                                  size_t length) {
  // TRACE_SCOPE("QuickJSRuntime", "string");
  JSValue jsValue = JS_NewStringLen(context_, str, length);
  ScopedJSValue scopedJsValue(context_, &jsValue);
...
jsi::String QuickJSRuntime::createStringFromUtf8(const uint8_t *str,
                                                 size_t length) {
  // TRACE_SCOPE("QuickJSRuntime", "string");
...
jsi::Object
QuickJSRuntime::createObject(std::shared_ptr<jsi::HostObject> hostObject) {
  // TRACE_SCOPE("QuickJSRuntime", "object");
  ...
  if (JS_IsNull(proto)) {
    ...
    JS_NewClass(JS_GetRuntime(context_), jsClassID,
                &HostObjectProxy::kJSClassDef);
    JS_SetPropertyFunctionList(context_, proto,
                               HostObjectProxy::kTemplateInterceptor, 1);
    ...
    // JS_SetClassProto steals the reference, so we dup it before to keep our
    // local ref
    JS_SetClassProto(context_, jsClassID, JS_DupValue(context_, proto));
  }
  JSValue object = JS_NewObjectProtoClass(context_, proto, jsClassID);
...
QuickJSRuntime::getHostObject(const jsi::Object &object) {
  // TRACE_SCOPE("QuickJSRuntime", "object");
  ...
  JSValue jsValue = JSIValueConverter::ToJSObject(*this, object);
  ScopedJSValue scopedJsValue(context_, &jsValue);
  auto opaqueData = reinterpret_cast<OpaqueData *>(
      JS_GetOpaque(jsValue, HostObjectProxy::GetClassID()));
  assert(opaqueData);
  auto hostObjectProxy =
      reinterpret_cast<HostObjectProxy *>(opaqueData->hostData_);
  return hostObjectProxy->GetHostObject();
}

jsi::HostFunctionType &
QuickJSRuntime::getHostFunction(const jsi::Function &function) {
  ...
  auto opaqueData = reinterpret_cast<OpaqueData *>(
      JS_GetOpaque(jsFunction, HostObjectProxy::GetClassID()));
  assert(opaqueData);
  auto hostFunctionProxy =
      reinterpret_cast<HostFunctionProxy *>(opaqueData->hostData_);
  return hostFunctionProxy->GetHostFunction();
}

jsi::Value QuickJSRuntime::getProperty(const jsi::Object &object,
...
bool QuickJSRuntime::hasProperty(const jsi::Object &object,
                                 const jsi::PropNameID &name) {
  // TRACE_SCOPE("QuickJSRuntime", "object");
  auto jsValue = JSIValueConverter::ToJSObject(*this, object);
  ScopedJSValue scopeValue(context_, &jsValue);
...
void QuickJSRuntime::setPropertyValue(const jsi::Object &object,
                                      const jsi::PropNameID &name,
...
jsi::Array QuickJSRuntime::getPropertyNames(const jsi::Object &object) {
  ...
  uint32_t size;
  JS_GetOwnPropertyNames(context_, &names, &size, jsValue,
  ...
  return make<jsi::Object>(new QuickJSPointerValue(runtime_, context_, result))
      .getArray(*this);
}

jsi::WeakObject
QuickJSRuntime::createWeakObject(const jsi::Object &weakObject) {
  ...
  return make<jsi::WeakObject>(
      new QuickJSPointerValue(runtime_, context_, value));
}
...
jsi::Array QuickJSRuntime::createArray(size_t) {
  // TRACE_SCOPE("QuickJSRuntime", "array");
  auto result = JS_NewArray(context_);
  ScopedJSValue scopeResult(context_, &result);
...
void QuickJSRuntime::setValueAtIndexImpl(const jsi::Array &array, size_t i,
                                         const jsi::Value &value) {
  // TRACE_SCOPE("QuickJSRuntime", "array");
  auto jsValue = JSIValueConverter::ToJSArray(*this, array);
  ScopedJSValue scopeValue(context_, &jsValue);
...
jsi::Function QuickJSRuntime::createFunctionFromHostFunction(
    const jsi::PropNameID &, unsigned int, jsi::HostFunctionType func) {
  // TRACE_SCOPE("QuickJSRuntime", "object");
...
jsi::Value QuickJSRuntime::call(const jsi::Function &function,
  ...
  auto jsObject = jsThis.isUndefined()
                      ? JS_GetGlobalObject(context_)
...
jsi::Value QuickJSRuntime::callAsConstructor(const jsi::Function &function,
...
bool QuickJSRuntime::strictEquals(const jsi::Symbol &a,
...
bool QuickJSRuntime::strictEquals(const jsi::String &a,
...
bool QuickJSRuntime::strictEquals(const jsi::Object &a,
...
void QuickJSRuntime::setExternalMemoryPressure(const jsi::Object &, size_t) {
  // QuickJS doesn't have external memory pressure API
}

void QuickJSRuntime::queueMicrotask(const jsi::Function &callback) {
  // QuickJS uses JS_EnqueueJob for microtasks, but we can just call the
  // function For simplicity, we'll execute it immediately
  call(callback, jsi::Value::undefined(), nullptr, 0);
}

jsi::PropNameID
QuickJSRuntime::createPropNameIDFromSymbol(const jsi::Symbol &sym) {
